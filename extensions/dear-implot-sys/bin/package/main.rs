...
fn locate_sys_out_dir(workspace_root: &std::path::Path, target: &str) -> Result<PathBuf, String> {
    ...
    let mut candidates: Vec<PathBuf> = match std::fs::read_dir(&build_root) {
        Ok(rd) => rd
            ...
            .filter_map(|e| {
                ...
                if name.starts_with("dear-implot-sys-") {
                    let out = p.join("out");
    ...
    if candidates.is_empty() {
        return Err(format!(
            "No dear-implot-sys build out directories found under {}",
            build_root.display()
        ));
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if implot_include.exists() {
        tar.append_dir_all("include/implot", &implot_include)?;
        println!("Added include/implot: {}", implot_include.display());
    } else {
        eprintln!(
