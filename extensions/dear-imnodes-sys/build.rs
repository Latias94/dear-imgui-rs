...
fn extension_artifact_profile(
    ...
    build_support::binding::extension_artifact_profile_from_env(
        build_support::binding::ExtensionBinding::ImNodes,
        &cfg.manifest_dir,
...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=IMNODES_SYS_CACHE_DIR");
    ...
    let force_build = cfg!(feature = "package-bin")
        ...
        || env::var("IMNODES_SYS_FORCE_BUILD").is_ok();
...
fn try_link_prebuilt(dir: PathBuf, cfg: &BuildConfig) -> bool {
    ...
    extension_artifact_profile(cfg, false)
        ...
        .unwrap_or_else(|error| {
            panic!("dear-imnodes-sys: incompatible prebuilt artifact: {error}")
        });
...
fn try_download_prebuilt_from_release(cfg: &BuildConfig) -> Option<PathBuf> {
    ...
    let tags = build_support::release_tags("dear-imnodes-sys", &profile.version);
