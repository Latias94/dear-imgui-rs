...
fn main() {
    ...
    // Try prebuilt then build
    let linked = try_link_prebuilt_all(&cfg);
...
fn try_download_prebuilt(
    ...
    if url.ends_with(".tar.gz") || url.ends_with(".tgz") {
        ...
        if !archive_path.exists() {
            println!(
                "cargo:warning=Downloading prebuilt dear_imnodes archive from {}",
                url
            );
...
fn try_download_prebuilt_from_release(cfg: &BuildConfig) -> Option<PathBuf> {
    ...
    let tags = [
        format!("dear-imnodes-sys-v{}", version),
        format!("v{}", version),
    ];
