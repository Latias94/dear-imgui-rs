...
thread_local! {
    ...
    static CURRENT_RESPONSE: RefCell<Vec<*mut ReflectResponse>> = RefCell::new(Vec::new());
    ...
    static CURRENT_FIELD_PATH: RefCell<Vec<String>> = RefCell::new(Vec::new());
}

fn with_response<R, F>(response: &mut ReflectResponse, f: F) -> R
where
    F: FnOnce() -> R,
{
...
fn current_field_path() -> Option<String> {
...
{
    CURRENT_FIELD_PATH.with(|stack| {
        stack.borrow_mut().push(segment.to_owned());
        let result = f();
        stack.borrow_mut().pop();
        result
    })
...
fn record_event(event: ReflectEvent) {
    CURRENT_RESPONSE.with(|stack| {
...
pub fn input_with_response<T: ImGuiReflect>(
    ...
    with_response(response, || input(ui, label, value))
}
    ...
    T: ImGuiValue,
{
    ...
    if let Some((from, to)) = move_op {
        if from < N && to < N && from != to {
            value.swap(from, to);
            record_event(ReflectEvent::ArrayReordered {
    ...
    S: BuildHasher,
{
    ...
    if let Some(_popup) = ui.begin_popup(&popup_id) {
        ...
        if ui.button("Add") {
            if !key_buf.is_empty() && !map.contains_key(key_buf) {
                ...
                record_event(ReflectEvent::MapInserted {
                    path: current_field_path(),
                    key: key_buf.clone(),
                });
                key_buf.clear();
    ...
    if clear_all {
        ...
        map.clear();
        record_event(ReflectEvent::MapCleared {
            path: current_field_path(),
            previous_len,
        });
        changed = true;
        ...
        if let Some(k) = key_to_remove {
            map.remove(&k);
            record_event(ReflectEvent::MapRemoved {
                path: current_field_path(),
                key: k,
            });
            changed = true;
        ...
        for (old_key, new_key) in rename_ops {
            ...
            if let Some(value) = map.remove(&old_key) {
                ...
                record_event(ReflectEvent::MapRenamed {
                    path: current_field_path(),
                    from: old_key,
                    to: new_key,
                });
    ...
    T: ImGuiValue + Default,
{
    ...
    if let Some((from, to)) = move_op {
        ...
        if from < len && to < len && from != to {
            ...
            record_event(ReflectEvent::VecReordered {
                path: current_field_path(),
                from,
                to: insert_index,
            });
