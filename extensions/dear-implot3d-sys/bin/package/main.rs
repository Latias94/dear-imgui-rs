...
fn locate_sys_out_dir(workspace_root: &std::path::Path, target: &str) -> Result<PathBuf, String> {
    ...
    let mut candidates: Vec<PathBuf> = match std::fs::read_dir(&build_root) {
        Ok(rd) => rd
            ...
            .filter_map(|e| {
                ...
                if name.starts_with("dear-implot3d-sys-") {
                    let out = p.join("out");
    ...
    if candidates.is_empty() {
        return Err(format!(
            "No dear-implot3d-sys build out directories found under {}",
            build_root.display()
        ));
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(v) = env::var("IMPLOT3D_SYS_PKG_CRT")
        && !v.is_empty()
    {
    ...
    let mut features = env::var("IMPLOT3D_SYS_PKG_FEATURES")
    ...
    let pkg_dir = env::var("IMPLOT3D_SYS_PACKAGE_DIR")
    ...
    println!("Packaging dear-implot3d prebuilt:");
    ...
    // Include headers: implot3d headers + cimplot3d.h
    let cimplot3d_root = manifest_dir.join("third-party").join("cimplot3d");
    let implot3d_include = cimplot3d_root.join("implot3d");
    if implot3d_include.exists() {
        append_headers_only(
            &mut tar,
            &implot3d_include,
            "include/implot3d",
            &["example", ".github", "docs", "misc"],
        )?;
        println!(
            "Added filtered include/implot3d headers from: {}",
            implot3d_include.display()
        );
    } else {
        eprintln!(
            "WARN: implot3d include dir not found: {}",
            implot3d_include.display()
        );
    }
    let cimplot3d_h = cimplot3d_root.join("cimplot3d.h");
    if cimplot3d_h.exists() {
        let mut f = fs::File::open(&cimplot3d_h)?;
        tar.append_file("include/cimplot3d/cimplot3d.h", &mut f)?;
        println!(
            "Added include/cimplot3d/cimplot3d.h: {}",
            cimplot3d_h.display()
        );
    } else {
        eprintln!("WARN: cimplot3d.h not found: {}", cimplot3d_h.display());
    }
    ...
    append_license_if_exists(
        &mut tar,
        &cimplot3d_root.join("LICENSE"),
        "licenses/cimplot3d-LICENSE",
    )?;
    append_license_if_exists(
        &mut tar,
        &cimplot3d_root.join("implot3d").join("LICENSE"),
        "licenses/implot3d-LICENSE",
    )?;
