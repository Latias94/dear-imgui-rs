...
impl<'ui> NodeEditor<'ui> {
    pub(crate) fn begin(ui: &'ui Ui, ctx: &'ui Context, editor: Option<&EditorContext>) -> Self {
        unsafe {
            sys::imnodes_SetImGuiContext(imgui_sys::igGetCurrentContext());
            sys::imnodes_SetCurrentContext(ctx.raw);
        }

        let editor_raw = editor.map(|ed| ed.raw);
...
pub struct PostEditor {
    ...
    link_created: Option<crate::LinkCreated>,
    link_created_ex: Option<crate::LinkCreatedEx>,
    link_destroyed: Option<i32>,
    any_attribute_active: Option<i32>,
    link_started: Option<i32>,
    link_dropped_excluding_detached: Option<i32>,
    link_dropped_including_detached: Option<i32>,
    _not_send_sync: PhantomData<Rc<()>>,
}
...
impl<'ui> NodeEditor<'ui> {
    ...
    pub fn end(mut self) -> PostEditor {
        ...
        PostEditor {
            ctx_raw: self.ctx_raw,
            editor_raw: self.editor_raw,
            editor_hovered,
            hovered_node,
            hovered_link,
            hovered_pin,
