...
- WGPU and Ash renderer callback registration and shutdown are ownership-aware. These renderer backends refuse occupied `Renderer_*` slots, preserve foreign replacements, and clear only callbacks and `RendererUserData` they still own. Ordered shutdown validates the complete callback table before touching platform windows and returns an ownership error without partial teardown when an active runtime's callbacks have drifted. Platform backends still own the complete `Platform_*` table exclusively and use bulk platform teardown.
- Change `dear-imgui-bevy::ImguiViewportBridge::commands()` from `&[ImguiViewportCommand]` to `Ref<'_, [ImguiViewportCommand]>` so the Context can retain the bridge allocation while native callbacks are installed without exposing a slice whose owner may move or drop. Iteration is unchanged; pass `&*bridge.commands()` when an API requires a literal slice.
...
- Require explicit ordered multi-viewport shutdown through the owning runtime instead of manually clearing callbacks. Split platform/renderer integrations destroy secondary viewports while both callback tables are live, then release renderer resources before the platform runtime; the paired SDL3 runtime performs its upstream-required platform shutdown before renderer shutdown. Context attachments preserve each backend's required ordering as a last-resort best-effort path, while explicit shutdown reports callback ownership drift, deferred callback faults, or incomplete GPU retirement.
...
- Keep ImPlot3D's high-level plot builders on contiguous Rust slices with safe circular offsets; remove their arbitrary-stride setters and make the explicit-layout `Plot3DUi::*_raw`, `surface_f32_flat`, and array-backed next-item style paths unsafe. Make ImPlot's arbitrary-layout/stride builder methods and array-backed next-item styles unsafe for the same reason. Upstream receives only base pointers, counts, and byte strides, so callers of those escape hatches must guarantee every computed coordinate and style-array read is initialized, aligned, in bounds, and alive for the submission.
...
- Reconcile every live font-atlas texture allocation across dynamic `FontSizeBase` changes, including overlapping create/update/destroy requests and out-of-order detached-renderer feedback, and keep SDL3 platform/renderer callbacks alive until secondary viewport teardown clears native user data. This fixes both the render panic while resizing fonts and the close-time viewport assertion reported in [#49](https://github.com/Latias94/dear-imgui-rs/issues/49), thanks [@TheDaChicken](https://github.com/TheDaChicken).
- Validate multi-viewport backend flags, required callbacks, monitor geometry, main-viewport initialization, and frame ordering before entering native window updates. Late enablement after a completed disabled frame now advances the native platform watermark, invalid second-frame enablement fails in Rust, main-viewport identity no longer depends on transient flags, and Bevy keeps callback state alive through Context-owned teardown.
