...
Pending publication. This alpha release candidate is published only after its release tag, date, and crates.io installation guidance are finalized.
...
| Multi-viewport backends | Winit, SDL3, Glow, WGPU, Ash, and Bevy use owning platform/renderer runtimes with explicit recovery and ordered shutdown. | Integrations no longer preserve callback state through caller-address stability, and surface, swapchain, device-loss, and teardown failures become reportable. |
...
- Give every `Context` a stable `ContextId` and shared `ContextBinding`, and route safe context-dependent operations through that binding instead of relying on whichever native context happens to be current. Custom integrations can register typed extension `ContextAttachment`s plus at most one renderer and one platform role; hooks return `Result`, and context teardown quiesces callbacks, releases renderer resources, destroys platform windows, and tombstones Rust state in that order. Final Drop notifies every peer in the current phase, then aborts before a later destructive phase if any hook errors or panics.
...
- Add idempotent `Context::end_frame()` for engine-owned teardown and use the same native close sequence from `FrameToken`, platform-window destruction, backend detachment, and `Context::drop`. Remove the public `MAIN_VIEWPORT_ID` numeric constant; use `Viewport::is_main()` for live data and `ViewportDrawDataSnapshot::is_main()` for detached data because numeric IDs are not cross-Context identities.
- Change `Context::destroy_platform_windows()` to return `Result<(), ContextPlatformWindowTeardownError>` and coordinate a platform attachment preflight/postflight transaction. A preflight error leaves native windows untouched but ends an open frame; a postflight error means native destruction has already occurred. On Winit, this detaches `WinitPlatformRuntime` while the base platform and Context remain usable; create a new runtime before resuming multi-viewport work. Prefer `WinitPlatformRuntime::shutdown` when backend-specific errors are required.
- Remove raw safe access to the `PlatformIO.Monitors` vector. Platform backends replace monitors through `unsafe PlatformIo::set_monitors()`, which validates geometry and allocates through Dear ImGui; the call remains unsafe because each optional native `PlatformHandle` must match the installed callbacks and outlive the monitor entry.
- Make raw `Io::set_backend_*_user_data`, `PlatformIo` callback mutation and clearing, backend-owned `Viewport` setters, legacy `FontAtlas::set_texture_id`, and renderer-owned `TextureData::{set_status,set_tex_id,set_backend_user_data}` mutation explicitly unsafe. Managed custom renderers should return request-bound `TextureFeedback`; raw backends must uphold callback ABI, pointer lifetime, texture state transitions, GPU synchronization, handle lineage, and teardown ordering.
- Route all seven aggregate-bearing PlatformIO callbacks through repository-owned C++ ABI shims: `Platform_SetWindowPos`, `Platform_GetWindowPos`, `Platform_SetWindowSize`, `Platform_GetWindowSize`, `Platform_GetWindowFramebufferScale`, `Platform_GetWindowWorkAreaInsets`, and `Renderer_SetWindowSize`. Raw Rust callbacks now receive aggregate inputs by pointer and return aggregates through out-parameters, avoiding compiler-dependent C++ small-aggregate calling conventions on MSVC. Typed callbacks retain their ergonomic value types through Rust trampolines. Native prebuilts must advertise `platform-io-aggregate-hooks-v2`; incompatible archives are rejected before linking.
- Glow, WGPU, and Ash renderer callback registration and shutdown are ownership-aware. These renderer backends refuse occupied `Renderer_*` slots or an existing `RENDERER_HAS_VIEWPORTS` claim, preserve foreign callback replacements, clear only callbacks and `RendererUserData` they still own, and immediately revoke their viewport capability when the table drifts. Ordered shutdown validates the complete callback table before touching platform windows and returns an ownership error without partial teardown. Platform backends transactionally claim the window-lifecycle `Platform_*` contract, validate it again before native teardown, and never pass their owned viewport state to a replacement callback; clipboard, shell, and IME service hooks remain composable for extensions such as Dear ImGui Test Engine.
- Change `dear-imgui-bevy::ImguiViewportBridge::commands()` to return an owned `Vec<ImguiViewportCommand>`, so observers cannot hold a `RefCell` borrow that blocks native callbacks. ECS consumption remains explicit through `drain_commands()`; callback contention is latched as a sticky Rust-side error without unwinding through C or consuming a partial queue. `ImguiContext::context_mut()` now rejects access while a scheduled `Ui` is live, and retryable `into_inner()` returns the still-owned wrapper until the render world has released managed GPU resources and every deferred secondary-window or camera despawn is observed after `ApplyDeferred`. Renderer ownership is preflighted before ending a frame or requesting release: a partial foreign takeover returns typed `RendererOwnership`; a complete takeover preserves the foreign raw fields but resets Bevy-managed texture bindings after releasing the GPU map, so the new renderer receives fresh create requests.
...
- Make Glow, WGPU, Ash, SDL3, Bevy, and dear-app consume Context-owned render work and key managed GPU resources by opaque `SnapshotTextureId`. Every renderer seals an identity before Destroy, acknowledges repeated Destroy idempotently, ignores late Create/Update without reviving a resource, and retains each tombstone until the contiguous completion watermark covers its destroy epoch. Custom renderers must prepare `Context::prepare_renderer_texture_reset(&consumer)` while their GPU map is intact, release that map, then commit the permit; dropping an uncommitted permit leaves native bindings unchanged. Ash additionally keeps fence-aware `TextureRetirementBatch` retirement so command recording is never mistaken for GPU completion.
- Store each Bevy managed GPU texture once under its `SnapshotTextureId`; the uploaded legacy `TextureId` is now an internal reverse alias instead of a duplicate bind-group entry. `ImguiTextureBindGroups::{insert,remove}` return `ImguiTextureBindGroupError` rather than overwriting or deleting an active managed alias.
- Default `dear-imgui-wgpu` to WGPU 30 and keep WGPU 29, 28, and 27 behind separate mutually exclusive compatibility features.
- Replace address-based multi-viewport `enable` functions with Context-bound owning runtimes. `WinitPlatform` owns the shared main `Arc<Window>` and the sole platform attachment; `WinitPlatformRuntime` borrows that owner to claim the complete callback table and manage secondary windows. WGPU's `WinitViewportRuntime` and `Sdl3ViewportRuntime` attach safely because their runtime owns callback-visible renderer storage. Glow's `GlowViewportRuntime::attach` remains unsafe because callers must prove the current GL context and share-group lineage; Ash exposes unsafe owning attach functions because Rust cannot prove raw Vulkan handle, device, queue, surface, and fence lineage. Callers no longer pin renderer storage themselves.
- Make `WinitPlatform::new`, `attach_window`, event/frame preparation, cursor and IME updates fallible. `attach_window` is now safe and consumes `Arc<Window>`, so the platform itself guarantees that the exact IME-visible allocation remains alive; construct a viewport runtime with `WinitPlatformRuntime::new(&mut Context, &WinitPlatform)` after attaching that same Context and window. `Rounded` and `Locked` HiDPI modes remain rejected for multi-viewport until primary and secondary coordinate strategies can be represented consistently.
- Require explicit ordered multi-viewport shutdown through the owning runtime instead of manually clearing callbacks. Winit platform shutdown now takes `&mut Context`; Winit and SDL3 validate the owning Context and end any open frame while callbacks are still attached before destroying secondary windows or native renderer state. Split platform/renderer integrations then release renderer resources before the platform runtime, while Drop defers native work to the Context attachment's deterministic teardown phases. Explicit shutdown reports callback ownership drift, deferred callback faults, or incomplete GPU retirement.
...
- Make every safe ImPlot3D surface path validate non-empty axes, checked `x_count * y_count`, the C++ `int` domain, string input, and exact X/Y/Z lengths before FFI. Axis-vector inputs flatten to full row-major coordinate arrays; `surface_f32_flat` is the safe contiguous allocation-reuse path, while arbitrary offset/stride submission exists only as `unsafe surface_f32_raw`. Surface builders now return `Plot3DError` instead of silently skipping invalid input. ImPlot's unrelated arbitrary-layout/stride builders and array-backed next-item styles remain unsafe for the same pointer-domain reason.
...
- Add ergonomic, non-truncating `TextureId` conversions: `u32`, `u64`, `usize`, non-zero integers, and pointers convert into application-owned IDs; conversion back to `u32` or `usize` uses `TryFrom` or `try_as_*`. `ManagedTextureId` remains intentionally opaque because its Context and slot generation are part of its identity.
...
- Add native Winit/WGPU and SDL3/Glow runtime smokes that create a real secondary OS viewport, render it, merge or destroy it, verify ordered teardown, and emit machine-validated evidence instead of treating headless coverage as a window-system substitute.
...
- Regenerate and verify core plus every maintained extension from canonical `CrateBindingSpec` profiles instead of per-crate hard-coded clang defines. Bindgen and native compilation consume the same ordered layout contract, bindgen-only macros stay out of C++, and disabled boolean macros are omitted rather than propagated as `-DNAME=0` where `#ifdef` would enable them.
...
- Gate release candidates through one clean committed local check and a same-SHA remote aggregate with 14 fixed cells covering Linux Test Engine, independent Winit/WGPU and SDL3/Glow real multi-viewport runtimes, Linux WASM, Windows vcpkg/MSVC MD/MSVC MT/MinGW, macOS, and five prebuilt targets. Python orchestration records commands, outcomes, target/CRT/toolchain metadata, binding and manifest hashes, runtime diagnostics, candidate SHA, and evidence SHA256 values; missing, skipped, cancelled, timed-out, failed, duplicate, stale, or wrong-SHA evidence is `No-Go`. Native runtime CI passes the trigger commit explicitly, and Release Gate rejects a dispatch whose workflow revision differs from the candidate before any evidence cell runs.
...
- Bind renderer-reset commits back to their owning Context even when another Context is temporarily current. Full texture replacement no longer clamps dimensions into Dear ImGui's `u16` update rectangle: initial Create requests retain their full dimensions, while an unrepresentable later full Update is rejected before pixels or status mutate.
- Track a newly created Glow texture before calling an application `TextureMap`, so an unwind cannot orphan the GL handle and explicit teardown can reclaim it after clearing partial map state.
- Reconcile every live font-atlas texture allocation across dynamic `FontSizeBase` changes with a pointer-free Rust ledger, content-stable revisions, idempotent Destroy tombstones, and ordered completion watermarks. Stale upload feedback cannot overwrite newer atlas state; SDL3 keeps aggregate platform pass-through callbacks inside C++, crosses Rust for its wrapped renderer-size callback only by pointer, and keeps callbacks alive until secondary viewport teardown clears native user data. This fixes the MSVC first-frame access violation, the render panic while resizing fonts, and the close-time viewport assertion reported in [#49](https://github.com/Latias94/dear-imgui-rs/issues/49), thanks [@TheDaChicken](https://github.com/TheDaChicken).
- Keep the `sdl3_sdlgpu_multi_view` example responsive during native Windows move and resize loops by using SDL main callbacks and a nonblocking main swapchain acquire; its shutdown now waits for idle GPU work before ordered backend teardown and main-window release. PR #53.
- Validate multi-viewport backend flags, required callbacks, monitor geometry, main-viewport initialization, and frame ordering before entering native window updates. Winit construction now prepares and validates all fallible state before publishing callbacks or raw user data and rolls every publication stage back on failure; Bevy callback contention becomes a deferred Rust error, and all teardown paths normalize an open frame before callbacks or windows are removed.
- Resolve Winit, WGPU, Ash, and Bevy viewport teardown through Dear ImGui's complete internal viewport registry instead of the filtered public `PlatformIO.Viewports` snapshot. Hidden, inactive, and zero-sized viewports now retain correct ownership validation and sidecar cleanup; stale or ID-reused native pointers are never dereferenced, while foreign field replacements remain fail-closed.
- Make WGPU secondary-surface scheduling and composition a single `WgpuViewportSurfaceConfig` carried through creation and surface-loss recovery. Applications can inherit the main surface's present mode, alpha mode, and frame latency; unsupported present/alpha choices use portable fallback policies instead of forcing `Fifo`. WGPU 30 secondary surfaces enforce the renderer's actual sRGB output contract and reject formats without sRGB presentation support instead of falsely advertising HDR or wide-gamut conversion. Fixes [#51](https://github.com/Latias94/dear-imgui-rs/issues/51), thanks [@DrBarnabus](https://github.com/DrBarnabus).
- Apply the same explicit secondary-window policy to Ash, Bevy, and SDL3: Ash selects complete Vulkan format/color-space pairs and VSync intent per surface, while Bevy carries present mode, compositor alpha, frame latency, theme, and real transparent clears through viewport creation. Winit no longer advertises a no-op opacity callback, routes secondary touch input, filters delayed programmatic move/resize events, applies no-input hit testing, honors taskbar suppression on Windows and X11, and rejects mixed-DPI layouts that its current logical coordinate model cannot represent. SDL3 now treats OpenGL and SDL_GPU viewport work as fail-closed native transactions, offers explicit immediate, VSync, adaptive, and main-window-matching GL swap policies, restores the caller's GL state, preserves foreign renderer callbacks during shutdown, retains its renderer consumer through Context-owned teardown, and commits texture reset before the full renderer shutdown can destroy platform windows. dear-app now rejects unsupported platform-viewports configuration and fully releases the old renderer before device-loss replacement.
- Keep SDL3 multi-viewport ownership scoped to native window-lifecycle callbacks, allowing Test Engine and other extensions to temporarily replace clipboard, shell, or IME service hooks without disabling viewport teardown. SDL3 now treats a rejected secondary OpenGL swap interval as a presentation fallback after context validation, and Winit treats `NO_FOCUS_ON_APPEARING` as a best-effort window-manager hint instead of rejecting an otherwise valid secondary window. WGPU's runtime example now transfers every attached backend into its owning application before fallible Test Engine setup, ends open frames during cleanup, attempts every ordered shutdown phase, and keeps the Context alive until all dependent owners have dropped.
- Compile core, backend shims, SDL3, and extension source/CMake paths with the same layout-affecting Dear ImGui defines used by generated bindings, and include those defines in prebuilt artifact identity. This prevents `ImGuiIO`/`ImGuiPlatformIO` offset drift and the resulting runtime layout assertion when an independently compiled C++ backend initializes.
...
Use `SharedFontAtlas::create()` only when multiple legacy-rendered contexts intentionally share one standalone atlas. A managed renderer needs one atlas registered to exactly one Context; give independent managed renderers separate atlases. A sole Context may enter managed mode on a shared owner, but a second registration is rejected. Before dropping the managed Context, prepare the renderer reset, release the complete GPU texture map, and commit the permit. Skipping that transaction preserves the old native binding and makes later registration return `SharedFontAtlasRendererReleasePending`; release the external resources, then drop and recreate the atlas. Both context-owned and shared atlases are borrowed through `font_atlas()`; mutating methods validate Dear ImGui's atlas lock internally.

`Glyph` is an owned metrics snapshot and intentionally has no safe UV accessor: lazy loading can repack the texture again within the same frame. Renderer integrations should read pixels and metadata through the `FontAtlasTexture<'_>` returned by `ctx.font_atlas().tex_data()`, drop that lease before rebuilding the atlas or advancing a frame, and query a legacy handle through `texture_id()`. Prefer request-bound `TextureFeedback`; a legacy renderer may report its own synchronized GPU handle through `unsafe { ctx.font_atlas().set_texture_id(texture_id) }`.
...
Create one renderer per Context. Passing a frame from another Context returns a context-mismatch error before mutation. `remove_texture` begins retirement rather than immediately reusing the slot; the renderer's request-bound feedback and completion watermark determine when reuse is safe. A custom renderer must call `ctx.prepare_renderer_texture_reset(&consumer)` with the same non-cloneable `RendererConsumer` that processed the frames while its complete device-side texture map still exists, release that map after the returned permit succeeds, and then commit the permit.
...
platform.attach_window(Arc::clone(&window), HiDpiMode::Default, &mut ctx)?;
let mut platform_runtime =
    WinitPlatformRuntime::new(&mut ctx, &platform)?;
...
platform_runtime.shutdown(&mut ctx)?;
platform.shutdown(&mut ctx)?;
```

WGPU attaches safely because the runtime owns address stability. Glow uses the same owning shape but `GlowViewportRuntime::attach` remains unsafe because the caller must prove current-GL-context and share-group lineage. Ash `WinitViewportRuntime::attach` and `Sdl3ViewportRuntime::attach` remain unsafe because the caller must prove raw Vulkan device, queue, surface, and synchronization lineage. For Ash, command recording is not GPU completion: acknowledge a `TextureRetirementBatch` only after the covering fences signal or the device is idle.

Do not compare a viewport ID with `0x11111111`. Use `viewport.is_main()` while the source Context is bound, or `viewport_snapshot.is_main()` after detaching draw data.

#### Bevy command observation and retryable extraction

`ImguiViewportBridge::commands()` now returns an owned `Vec`, so application inspection cannot block callback enqueueing:

```rust
let observed = viewport_bridge.commands();
for command in &observed {
    inspect(command);
}
```

`ImguiContext::context_mut()` is unavailable while `ImguiPrimaryContextPass` exposes a live `Ui`. When consuming the wrapper, handle `ImguiContextIntoInnerError` by recovering `into_owner()`: run the Bevy render schedule for `RenderWorldReleasePending`, run the main schedule through deferred entity application for `ViewportWorldReleasePending`, or restore/complete the external renderer contract named by `RendererOwnership`, then retry `into_inner()`. Failed ownership preflight does not end the frame, request renderer release, or reset native texture bindings. A complete external renderer takeover preserves its raw fields, but Bevy-managed texture IDs are reset because raw callback replacement does not transfer the released Bevy GPU map.

`ImguiBackendConfig` now owns the secondary-window policy. Existing complete struct literals must
add `viewport_window` (or use an update expression):

```rust
let config = ImguiBackendConfig {
    multi_viewport: true,
    viewport_window: ImguiViewportWindowConfig::from_window(&primary_window),
    ..Default::default()
};
```

#### ImPlot3D surface validation

Safe surface submission is now fallible. Propagate or handle `Plot3DError`, use `surface_f32_flat` for already-expanded contiguous X/Y/Z arrays, and reserve `unsafe surface_f32_raw` for caller-proven offset/stride layouts:

```rust
Surface3D::new("height", &x_axis, &y_axis, &z_values).try_plot(&plot_ui)?;
```

Axis-vector APIs expand X and Y into the full row-major grid before FFI, so `z_values.len()` must equal `x_axis.len() * y_axis.len()`.
...
```toml
dear-imgui-wgpu = { version = "=0.16.0-alpha.1", features = ["tracing"] }
...
dear-implot = { version = "=0.16.0-alpha.1", features = ["build-from-source"] }

# Native pregenerated bindings plus a matching downloaded binary artifact.
dear-implot = { version = "=0.16.0-alpha.1", features = ["prebuilt"] }

# Browser import bindings; supported by ImPlot, ImPlot3D, ImNodes, ImGuizmo, and ImGuizmo Quat.
dear-implot = { version = "=0.16.0-alpha.1", features = ["wasm"] }
```
