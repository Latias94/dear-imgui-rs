...
- Make raw `PlatformIo` callback mutation and clearing, plus backend-owned `Viewport` user-data, native-handle, and window-created setters, explicitly unsafe. Prefer the owning backend runtimes; custom backends using these escape hatches must uphold callback ABI, function/user-data lifetime, handle lineage, and teardown ordering.
