...
| Infallible `Context::suspend`, owner-only `SuspendedContext::activate` errors, or `try_with_active` returning the closure's error directly | Handle `ContextSuspensionError` / `ContextActivationError` and recover the retained owner with `into_owner` or `into_parts`. `try_with_active` now returns `ScopedActivationError<E>`, separating `ContextScopeError` from `Closure(E)`. Use `suspend_or_panic`, `activate_or_panic`, or `with_active_or_panic` only when conflicts are programmer errors. |
| `DrawListTextNoPixelSnapToken`, table draw-channel tokens, or references to the unreachable `WindowToken` / `ChildWindowToken` exports | Use `with_text_no_pixel_snap`, `with_table_background_channel`, `with_table_column_channel`, and the window/child builders. Retained native stack tokens validate exact Context, frame, window/table provenance, and per-resource LIFO order before FFI; `StyleVar::Alpha` and effective disabled scopes also share restoration order, and a table cannot end while a scope created inside it is active. Closure helpers are the normal path. |
| A borrowed `TableSortSpecs` view or clearing its dirty bit without the producing `Ui` | Keep the owned `TableSortSpecs` snapshot for sorting, then call `clear_dirty(&ui)` while the original table is still current. Snapshot iteration remains valid after the table ends. |
...
| `RenderedFrame`, the mode-selecting `RendererConsumer`, or `Context::render()` without a consumer | Create a `SynchronousRendererConsumer` and call `Context::render(&consumer)` to obtain a non-drawable `PendingFrame`; reconcile every texture request to obtain the drawable `ReconciledFrame`. For render-thread handoff, create a `DetachedRendererConsumer` and send its move-only `FrameSnapshot`. Use `render_legacy()` only with renderers that do not advertise managed textures. |
| `WgpuRenderer::new_frame()`, runtime `new_frame()` calls, `reconcile_frame(&mut frame)`, or draw-data access before texture feedback | Pass `PendingFrame` by value. WGPU, Glow, and SDL3 can consume it through `render(...)`, or split work into `reconcile_frame(frame)` followed by `render_reconciled(...)`; Ash uses `prepare_frame(frame)` before command recording. Multi-viewport runtimes must reconcile before invoking renderer callbacks. |
...
| `TestFrameDriver::render`, `HeadlessRenderError`, or the alpha.1 `FrameDriverError` / `RunnerError` generic lists | Implement `prepare` -> `render_main` -> `present`: preparation consumes `FrameToken` and returns `ReconciledFrame`, main rendering returns `MainRenderOutcome`, and `drive_frame` returns `FrameDriveOutcome`. Rename the headless error to `HeadlessPrepareError` and add the driver's `PrepareError` to error types. |
| `FrameResult::rendered_frame`, or using the binding count returned by a renderer texture reset | Use `FrameResult::pending_frame` or `into_parts()`, then reconcile it before drawing. Successful `RendererTextureReset::commit` and `ContextTeardown::with_renderer_texture_reset` completion carry no count; handle only their fallible preparation or reset errors. |
| `OwnedTextureData::new` followed by `create` / `set_data`, manual texture metadata setters, `destroy_pixels`, or `ManagedTextureMut::set_data` | Construct with `OwnedTextureData::from_pixels`. Use `replace_pixels` for an exact full replacement, or `TextureRegion` plus `TextureSubresource` for a strided partial update. For managed textures, `Context::try_with_texture_mut` reports access and pixel-validation failures in one result. Each mutation call is atomic; a rejected call changes nothing, while an earlier successful call remains applied and invalidates stale renderer feedback even if a later call in the same closure fails. |
| `Viewport::draw_data_ref()` | Render through a `ReconciledFrame`, or capture a pointer-free `FrameSnapshot`. Custom renderers may read `Viewport::draw_data()` only inside an unsafe, current-frame native render boundary. |
...
- Scoped UI state now tracks the exact native resource stack plus frame, window `Begin`, table instance, and table-cell provenance. Invalid token order is rejected before FFI and recovered during unwind; additive indentation remains correct under arbitrary same-window drop order. New `main_menu_bar`, `menu_bar`, `with_disabled`, and `with_disabled_if` helpers make lexical scopes the default.
- Bevy preserves Context activation, open-frame, and binding failures as source-chained `ImguiContextError::ScopedActivation` values instead of flattening them into renderer or configuration errors.
