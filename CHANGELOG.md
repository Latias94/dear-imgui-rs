...
- Reconcile every live font-atlas texture allocation across dynamic `FontSizeBase` changes with a pointer-free Rust ledger, content-stable revisions, idempotent Destroy tombstones, and ordered completion watermarks. Stale upload feedback cannot overwrite newer atlas state, native list refreshes cannot dereference freed allocations, and SDL3 callbacks remain alive until secondary viewport teardown clears native user data. This fixes both the render panic while resizing fonts and the close-time viewport assertion reported in [#49](https://github.com/Latias94/dear-imgui-rs/issues/49), thanks [@TheDaChicken](https://github.com/TheDaChicken).
