...
- Glow, WGPU, and Ash renderer callback registration and shutdown are ownership-aware. These renderer backends refuse occupied `Renderer_*` slots or an existing `RENDERER_HAS_VIEWPORTS` claim, preserve foreign callback replacements, clear only callbacks and `RendererUserData` they still own, and immediately revoke their viewport capability when the table drifts. Ordered shutdown validates the complete callback table before touching platform windows and returns an ownership error without partial teardown. Platform backends claim the complete `Platform_*` contract transactionally, validate it again before native teardown, and never pass their owned viewport state to a replacement callback.
