...
- Replace cloneable or pointer-bearing detached output with one non-cloneable `RendererConsumer` per Context and a move-only, pointer-free `FrameSnapshot`. Create the consumer with `Context::create_renderer_consumer`, detach through `render_snapshot`, commit exactly one matching feedback batch on the render thread, and call `poll_snapshot_completions` on the UI thread; consumer generation, epoch, revision, request identity, abandon, and reset checks reject stale or duplicate completion.
