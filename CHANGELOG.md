...
- Keep SDL3 multi-viewport ownership scoped to native window-lifecycle callbacks, allowing Test Engine and other extensions to temporarily replace clipboard, shell, or IME service hooks without disabling viewport teardown. WGPU's runtime example now transfers every attached backend into its owning application before fallible Test Engine setup, ends open frames during cleanup, attempts every ordered shutdown phase, and keeps the Context alive until all dependent owners have dropped.
