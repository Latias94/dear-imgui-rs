...
| `DockBuilder`, `DockNode`, or `SplitDirection` | Submit a declarative `DockLayout` and `DockspaceTarget`; use `DockLayoutApply::IfMissing` for restored layouts and `Replace` only for an intentional reset. |
| Direct `WindowClass` field writes or a safe raw focus-route parent ID | Use `WindowClass` builders and read-only accessors. Raw platform icon data and focus-route parent IDs now require explicit `unsafe` calls with their native lifetime and topology contracts upheld. |
...
| Pinned renderer storage, free callback installers, manual callback clearing, or SDL3 renderer attach without a platform owner | Use the owning platform and renderer runtime, pass the live `Sdl3PlatformBackend` to SDL3 WGPU/Ash attach, and call the ordered, fallible shutdown path. |
...
- Winit and Bevy native multi-viewport docking now keep client coordinates, event ordering, capture handoff, decorated-window placement, and native move/resize feedback synchronized across mixed-DPI desktops. Wayland explicitly falls back to docking inside the host window because it cannot provide global desktop positions. Addresses [#60](https://github.com/Latias94/dear-imgui-rs/issues/60), reported by [@lysenika](https://github.com/lysenika).
