...
| Docking and viewports | Submit a validated `DockLayout` with `DockspaceOptions`, use `WindowKey` for stable identity, and treat viewport topology as read-only outside documented unsafe backend boundaries. | Layout replacement is transactional and docking identity survives title, viewport, and native-window changes. |
| Winit, WGPU, Glow, Ash, and SDL3 | Replace public manual trace/reconcile/callback phases with the owning route's `prepare(...)` or `prepare_frame(...)` transaction followed by `render_main(...)` or Ash `cmd_draw_main(...)`; shut down renderer ownership before platform ownership and native GPU/window objects. | Secondary dispatch, fault aggregation, GL restoration, Vulkan retirement, and teardown run in one ordered capability without public phase-skipping aliases. |
