...
- Added a checked unknown-count list-clipper protocol, bounded Test Engine runners with explicit graphical and virtual presentation paths, run-scoped framebuffer capture providers, typed lifecycle/results, label-based table resizing, docking automation, and terminal/error logging. Graphical drivers now consume each rendered frame once and own its render/present transaction.
...
- Winit and Bevy native multi-viewport docking now keep client coordinates, DPI-time event ordering, Context-wide focus, capture handoff, decorated-window placement, camera clear policy, and native move/resize feedback synchronized across mixed-DPI desktops. Bevy reports capability per Context; Wayland explicitly falls back to docking inside the host window because it cannot provide global desktop positions. Addresses [#60](https://github.com/Latias94/dear-imgui-rs/issues/60), reported by [@lysenika](https://github.com/lysenika).
