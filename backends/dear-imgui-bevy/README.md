...
The backend only advertises native platform viewports when the active Winit display provides global desktop client-area coordinates. Wayland intentionally reports `ImguiNativeViewportSupport::GlobalDesktopCoordinatesUnavailable`; native windows are then disabled for that Context while ordinary in-window docking continues. Hosts that need to explain this fallback can read the `ImguiNativeViewportSupport` resource. On startup, `PendingNativeWindow` means that the primary Winit window has not been registered yet.
