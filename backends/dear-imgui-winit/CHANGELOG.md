...
- Make explicit `WinitPlatform` and `WinitPlatformRuntime` shutdown transactional against the Context attachment graph. Shutdown now rejects an active renderer attachment before closing an open frame or changing native state; after renderer shutdown, the same platform generation can be retried, while Context-owned teardown keeps its existing renderer-before-platform ordering.
- Reconcile focus across all windows owned by one Context at frame boundaries. Focus transfers between primary and secondary viewports no longer emit a false application-wide loss, while a real loss clears stale mouse-button state so a missing release event cannot suppress later pointer-leave handling.
- Use Dear ImGui's native desktop coordinate contract throughout multi-viewport geometry and input. Windows and X11 publish physical virtual-desktop pixels with `FramebufferScale = (1, 1)`, macOS uses Cocoa points with its backing scale, and per-window `DpiScale` remains independent. Mixed-DPI layouts are supported, decorated Windows positions use the target viewport DPI, monitor publication refreshes at frame boundaries without taking ownership from another backend, and runtime attach/shutdown invalidates stale pointer coordinates while restoring the base single-window display contract.
- Stop advertising the unsupported `Platform_SetWindowAlpha` callback, synchronize cursor and IME state for new and existing secondary windows, route secondary-window touch input through the primary translator, and honor `NO_TASK_BAR_ICON` on Windows and X11. Windows `NO_INPUTS` viewports use native `WM_NCHITTEST` pass-through and per-frame native hovered-window discovery without changing compositor styles; moving viewports are raised above docking targets without activation, and mouse capture is transferred when a dragged viewport is destroyed. Other desktop targets use Winit cursor hit testing.
- Reject Wayland and non-desktop targets before publishing multi-viewport capability, fail closed for unsupported click-focus or taskbar semantics while treating `NO_FOCUS_ON_APPEARING` as a best-effort inactive-creation request, synchronize live decoration/top-most/taskbar policies where the platform supports them, and translate primary and secondary touches in the native desktop coordinate space with Context-local first-finger tracking. Native move, resize, and DPI notifications are authoritative platform feedback so decorated client origins cannot drift from the OS window.
