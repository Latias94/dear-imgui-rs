...
/// Secondary-viewport Vulkan submissions collected within one trace scope.
///
/// A viewport appears in [`Self::render_submitted_viewport_ids`] only after Ash submitted its
/// command buffer. It appears in [`Self::present_submitted_viewport_ids`] only after
/// `vkQueuePresentKHR` accepted that viewport's acquired image. Both ID sets are sorted and
/// contain no duplicates.
#[derive(Debug, Default, Eq, PartialEq)]
pub struct AshViewportFrameReport {
    render_submitted_viewport_ids: Vec<Id>,
    present_submitted_viewport_ids: Vec<Id>,
}

impl AshViewportFrameReport {
    ...
    /// Returns sorted, unique viewport IDs accepted for presentation.
...
pub(super) struct FrameTraceState {
    active: Option<AshViewportFrameReport>,
}
...
impl FrameTraceState {
    pub(super) fn begin(&mut self) -> bool {
        ...
        self.active = Some(AshViewportFrameReport::default());
        true
    }
    ...
    pub(super) fn finish(&mut self) -> AshViewportFrameReport {
        self.active
            .take()
            .expect("a live Ash frame-trace guard owns the active trace")
            .finish()
    }
