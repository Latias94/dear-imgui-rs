...
pub(super) fn preflight_callbacks(context: &Context) -> Result<(), GlowViewportError> {
    ...
    binding.with_bound_context(|| {
        ...
        {
            return Err(GlowViewportError::RendererViewportCapabilityOccupied);
        }
        ...
        if let Some(callback) = first_missing_platform_callback(raw) {
...
pub(super) fn detect_callback_drift(control: &RuntimeControl) {
    ...
    let _ = control
        .binding()
        .try_with_bound_context(|| runtime_dependencies_are_intact(control));
}

fn runtime_dependencies_are_intact(control: &RuntimeControl) -> bool {
    if !control.should_detect_callback_drift() {
        return false;
    }
    let fault = current_runtime_dependency_fault();
    let Some(fault) = fault else {
        return true;
    };

    clear_renderer_viewport_flag_if_callback_owned();
    control.record_dependency_fault(fault);
    false
}

fn current_runtime_dependency_fault() -> Option<GlowViewportError> {
    let io = unsafe { sys::igGetIO_Nil() };
    if io.is_null() {
        return Some(GlowViewportError::PlatformIoUnavailable);
    }
    let flags = BackendFlags::from_bits_truncate(unsafe { (*io).BackendFlags });
    if !flags.contains(BackendFlags::RENDERER_HAS_VIEWPORTS) {
        return Some(GlowViewportError::RendererViewportCapabilityLost);
    }
    if !flags.contains(BackendFlags::PLATFORM_HAS_VIEWPORTS) {
        return Some(GlowViewportError::PlatformBackendUnavailable);
    }
    ...
    // SAFETY: the caller bound this runtime's live Context for the entire check.
    let platform_io = unsafe { &*platform_io };
    if let Some(callback) = first_missing_platform_callback(platform_io) {
        return Some(GlowViewportError::PlatformCallbackUnavailable { callback });
    };
    first_renderer_callback_drift(platform_io)
        .map(|callback| GlowViewportError::RendererCallbackReplaced { callback })
}

fn clear_renderer_viewport_flag_if_callback_owned() {
    let platform_io = unsafe { sys::igGetPlatformIO_Nil() };
    if platform_io.is_null() || !runtime_owns_any_renderer_callback(unsafe { &*platform_io }) {
        return;
    }
    let io = unsafe { sys::igGetIO_Nil() };
    if !io.is_null() {
        let viewport_bit = BackendFlags::RENDERER_HAS_VIEWPORTS.bits();
        // SAFETY: the caller has bound this runtime's live Context.
        unsafe { (*io).BackendFlags &= !viewport_bit };
    }
}
...
pub(super) fn release_callbacks(control: &RuntimeControl) -> Result<(), GlowViewportError> {
    ...
    let owns_any_callback = runtime_owns_any_renderer_callback(raw);
    if owns_any_callback {
        unsafe { platform_io.set_renderer_render_window_raw(None) };
    }

    let io = unsafe { sys::igGetIO_Nil() };
    if owns_any_callback && !io.is_null() {
        let viewport_bit = BackendFlags::RENDERER_HAS_VIEWPORTS.bits();
        unsafe {
            (*io).BackendFlags &= !viewport_bit;
        }
    }
...
fn first_missing_platform_callback(raw: &sys::ImGuiPlatformIO) -> Option<&'static str> {
    [
        (raw.Platform_CreateWindow.is_none(), "Platform_CreateWindow"),
        (
            raw.Platform_DestroyWindow.is_none(),
            "Platform_DestroyWindow",
        ),
        (raw.Platform_RenderWindow.is_none(), "Platform_RenderWindow"),
        (raw.Platform_SwapBuffers.is_none(), "Platform_SwapBuffers"),
    ]
    .into_iter()
...
fn render_viewport(control: &RuntimeControl, viewport: *mut sys::ImGuiViewport) {
    ...
    control.with_renderer_callback("Renderer_RenderWindow", |renderer, gl| {
        ...
        let draw_data = (!viewport.DrawData.is_null()).then(|| {
            // SAFETY: DrawData is owned by this live viewport for the callback duration.
            let raw_draw_data = unsafe { &*viewport.DrawData };
            unsafe { DrawData::from_raw(raw_draw_data) }
        });
        renderer.render_viewport_draw_data(
            gl,
            draw_data,
...
pub(crate) unsafe extern "C" fn renderer_render_window_sys(
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        if !runtime_dependencies_are_intact(&control) {
            return;
        }
