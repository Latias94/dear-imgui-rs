...
impl WgpuRenderer {
    ...
    /// atlas PBO upload) and want to display it via legacy `TextureId` path:
    ...
    pub fn register_external_texture(
        ...
        // No bind group created yet; it will be created on first use lazily.
        id
    }
    ...
    pub fn update_external_texture_view(
        ...
        // Replace by remove + insert with same Texture handle and new view
        if let Some(mut tex) = self.texture_manager.remove_texture(texture_id) {
            ...
            // Invalidate cached bind group so it recreates with the new view
