...
use dear_imgui::{Context, DrawData, DrawList, Result};
...
struct DrawVertPod(sys::ImDrawVert);
...
impl WgpuRenderer {
        ...
        let pipeline = device.create_render_pipeline(&RenderPipelineDescriptor {
            ...
            vertex: VertexState {
                ...
                buffers: &[VertexBufferLayout {
                    array_stride: size_of::<sys::ImDrawVert>() as BufferAddress,
    ...
    pub fn prepare(
        ...
        let mut indices = Vec::with_capacity(index_count * std::mem::size_of::<sys::ImDrawIdx>());
    ...
    pub fn reload_font_texture(&mut self, imgui_ctx: &mut Context, device: &Device, queue: &Queue) {
        ...
        if texture_data.width > 0 && texture_data.height > 0 && !texture_data.data.is_empty() {
            // Create font texture
            ...
            // Set the texture ID in Dear ImGui
            fonts.set_tex_id(1);
    ...
    fn render_draw_list<'render>(
        ...
        for cmd in draw_list.commands() {
            // Skip user callbacks for now
            if cmd.UserCallback.is_some() {
                start += cmd.ElemCount;
                continue;
            }

            // Project scissor/clipping rectangles into framebuffer space (following official implementation)
            let clip_min_x = (cmd.ClipRect.x - clip_off[0]) * clip_scale[0];
            let clip_min_y = (cmd.ClipRect.y - clip_off[1]) * clip_scale[1];
            let clip_max_x = (cmd.ClipRect.z - clip_off[0]) * clip_scale[0];
            let clip_max_y = (cmd.ClipRect.w - clip_off[1]) * clip_scale[1];
            ...
            if clip_max_x > clip_min_x && clip_max_y > clip_min_y {
                ...
                if w > 0 && h > 0 {
                    ...
                    let texture_bind_group = if cmd.TexRef._TexID == 1 {
                        // Font texture
