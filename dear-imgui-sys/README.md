...
This crate provides unsafe Rust bindings to Dear ImGui v1.92.9 (docking branch) using the [cimgui](https://github.com/cimgui/cimgui) C API. The core `ig*` API crosses a C ABI boundary. C++ backend integration and callback-bearing platform APIs use explicit repository-owned shims because their compiler ABI still matters, especially on MSVC.
...
- **Modern Dear ImGui**: Based on Dear ImGui v1.92.9 docking branch
...
`0.16.0-alpha.1` archives are available only after that prerelease is published;
before then, use a repository checkout or build from source.
...
export IMGUI_SYS_PREBUILT_URL=/path/to/dear-imgui-prebuilt-0.16.0-alpha.1-<target>-static.tar.gz
...
Until `0.16.0-alpha.1` is published, test this candidate from `main`:
...
```toml
[dependencies]
dear-imgui-sys = "=0.16.0-alpha.1"

# Enable features as needed
dear-imgui-sys = { version = "=0.16.0-alpha.1", features = ["freetype", "wasm"] }
```
...
```toml
[dependencies]
dear-imgui-sys = { version = "=0.16.0-alpha.1", features = ["backend-shim-opengl3"] }
```
...
1. Custom Android backend without a dedicated first-party crate yet:
   ...
   dear-imgui-sys = { version = "=0.16.0-alpha.1", features = ["backend-shim-android", "backend-shim-opengl3"] }
   ```
...
- **Dear ImGui Version**: v1.92.9 (docking branch)
- **cimgui Version**: Pinned to a revision generated against Dear ImGui v1.92.9
