...
fn test_list_clipper(context: &mut Context) {
    ...
    let mut clipper_with_height = dear_imgui::widget::list_clipper::ListClipper::new_with_height(large_list_size, 20.0);
    println!("  - Created ListClipper with fixed height: {}", clipper_with_height.items_height());
