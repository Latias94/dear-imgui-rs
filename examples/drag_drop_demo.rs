...
fn main() {
    ...
    if let Some(target) = ui.drag_drop_target() {
        if target
            .accept_payload_empty("EMPTY_PAYLOAD", DragDropFlags::NONE)
            .is_some()
        {
    ...
    if let Some(source) = ui
        .drag_drop_source_config("TYPED_PAYLOAD")
    ...
    if let Some(target) = ui.drag_drop_target() {
        if let Some(Ok(payload)) =
            target.accept_payload::<i32, _>("TYPED_PAYLOAD", DragDropFlags::NONE)
        {
    ...
    if let Some(source) = ui
        .drag_drop_source_config("STRING_PAYLOAD")
    ...
    if let Some(target) = ui.drag_drop_target() {
        if let Some(Ok(payload)) =
            target.accept_payload::<&'static str, _>("STRING_PAYLOAD", DragDropFlags::NONE)
        {
...
mod tests {
    ...
    fn test_drag_drop_compilation() {
        ...
        assert_eq!(
            ...
            std::mem::size_of::<DragDropSource<&str>>()
        );

        // Test flags
        let flags =
            DragDropFlags::SOURCE_NO_PREVIEW_TOOLTIP | DragDropFlags::ACCEPT_BEFORE_DELIVERY;
    ...
    fn test_drag_drop_flags() {
        ...
        let source_flags =
            DragDropFlags::SOURCE_NO_PREVIEW_TOOLTIP | DragDropFlags::SOURCE_NO_DISABLE_HOVER;

        let target_flags =
            DragDropFlags::ACCEPT_BEFORE_DELIVERY | DragDropFlags::ACCEPT_NO_DRAW_DEFAULT_RECT;
