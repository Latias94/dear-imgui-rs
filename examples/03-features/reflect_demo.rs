        ...
        ui.window("Reflect Demo")
            ...
            .build(|| {
                ...
                if let Some(_node) = ui.tree_node("Primitives & Bool Styles") {
                    inspector.input("Primitives", &mut state.primitives);
                }

                if let Some(_node) = ui.tree_node("Text & Containers") {
                    inspector.input("Text/Containers", &mut state.text_and_containers);
                }

                if let Some(_node) = ui.tree_node("Maps & Tuples") {
                    inspector.input("Maps/Tuples", &mut state.maps_and_tuples);
                }

                if let Some(_node) = ui.tree_node("Pointers") {
                    inspector.input("Pointers", &mut state.pointers);
                }

                if let Some(_node) = ui.tree_node("Type-level Numerics") {
                    inspector.input("Type Defaults", &mut state.type_level_numerics);
                }

                if let Some(_node) = ui.tree_node("Enums") {
                    inspector.input("Quality", &mut state.quality);
                    inspector.input("Render Mode", &mut state.mode);
                }

                if let Some(_node) = ui.tree_node("glam Vectors") {
                    inspector.input("Glam", &mut state.glam);
                }

                if let Some(_node) = ui.tree_node("ReflectResponse Events") {
                    if inspector.response().is_empty() {
...
fn main() {
    ...
    let mut reflect_session = reflect::ReflectSession::new();
    configure_reflect_settings(&mut reflect_session);
