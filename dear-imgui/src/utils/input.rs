...
impl crate::ui::Ui {
    ...
    pub fn get_mouse_clicked_count(&self, button: MouseButton) -> usize {
        non_negative_count_from_i32(
            "Ui::get_mouse_clicked_count()",
            self.run_with_bound_context(|| unsafe { sys::igGetMouseClickedCount(button.into()) }),
        )
    }
    ...
    pub fn get_mouse_drag_delta(&self, button: MouseButton, lock_threshold: f32) -> [f32; 2] {
        ...
        let delta = self.run_with_bound_context(|| unsafe {
            sys::igGetMouseDragDelta(button.into(), lock_threshold)
        });
        [delta.x, delta.y]
    }
    ...
    pub fn is_any_mouse_down(&self) -> bool {
        self.run_with_bound_context(|| unsafe { sys::igIsAnyMouseDown() })
    }
}
