...
impl Ui {
    ...
    pub fn begin_tooltip(&self) -> Option<TooltipToken<'_>> {
        if unsafe { sys::ImGui_BeginTooltip() } {
...
impl Ui {
    ...
    pub fn is_item_hovered_with_flags(&self, flags: HoveredFlags) -> bool {
        unsafe { sys::ImGui_IsItemHovered(flags.bits()) }
    }
    ...
    pub fn is_item_clicked_with_button(&self, mouse_button: MouseButton) -> bool {
        unsafe { sys::ImGui_IsItemClicked(mouse_button as i32) }
    }
    ...
    pub fn item_rect(&self) -> ([f32; 2], [f32; 2]) {
        unsafe {
            let min = sys::ImGui_GetItemRectMin();
            let max = sys::ImGui_GetItemRectMax();
    ...
    pub fn item_rect_size(&self) -> [f32; 2] {
        unsafe {
            let size = sys::ImGui_GetItemRectSize();
            [size.x, size.y]
        }
    }
}
