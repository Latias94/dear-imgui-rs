use crate::context::binding::with_bound_context;
use crate::io::{Io, assert_positive_f32};
use crate::sys;
...
impl Io {
    ...
    pub fn font_global_scale(&self) -> f32 {
        with_bound_context(self.context_ptr("Io::font_global_scale()"), || unsafe {
    ...
    pub fn set_font_global_scale(&mut self, scale: f32) {
        ...
        with_bound_context(self.context_ptr("Io::set_font_global_scale()"), || unsafe {
            let style = sys::igGetStyle();
            assert!(
                !style.is_null(),
