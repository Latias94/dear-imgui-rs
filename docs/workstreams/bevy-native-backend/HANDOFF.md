...
- BEVY-110 simple example: `backends/dear-imgui-bevy/examples/basic/simple.rs` demonstrates installing `ImguiPlugin`, creating a primary window entity, and drawing overlay UI from `ImguiPrimaryContextPass`.
- BEVY-120 editor shell example: `backends/dear-imgui-bevy/examples/editor/editor_shell.rs` shows a Bevy render-target `Image` inside a dockspace-driven editor shell and documents the input routing policy expected for editor tools.
- BEVY-130 ecosystem example: `backends/dear-imgui-bevy/examples/ecosystem/ecosystem.rs` initializes ImPlot, ImNodes, and ImGuizmo as non-send Bevy resources and draws them through the same Bevy-managed ImGui frame.
