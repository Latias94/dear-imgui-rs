...
- [x] BEVY-100 [owner=codex] [deps=BEVY-090] [scope=backends/dear-imgui-bevy/src/texture.rs,backends/dear-imgui-bevy/src/render]
  ...
  Review: `review-workstream` before accepting completion.
  Evidence: `backends/dear-imgui-bevy/src/texture.rs`, `backends/dear-imgui-bevy/src/render.rs`, `backends/dear-imgui-bevy/tests/texture.rs`, fresh `cargo +stable nextest run -p dear-imgui-bevy --features render texture`, `cargo +stable nextest run -p dear-imgui-bevy --features render`, `cargo +stable nextest run -p dear-imgui-bevy`, `cargo +stable check -p dear-imgui-bevy --features render`, `cargo +stable check -p dear-imgui-bevy --no-default-features`, `cargo +stable clippy -p dear-imgui-bevy --all-targets --features render --no-deps -- -D warnings`, and `cargo +stable fmt --all --check`.
  Handoff: DONE 2026-05-23. Added `ImguiTextureFeedbackQueue` and main-thread feedback application before `ImguiBeginFrame`, shared the queue with Bevy render-world systems, added `ImguiBevyTextures` for stable `Handle<Image>` to `TextureId` registration, extracted those mappings into render world, and populated legacy Bevy image bind groups from `RenderAssets<GpuImage>`. Full render-to-texture viewport usage is now enabled for BEVY-120 examples; a runtime GPU bind-group test remains future CI coverage because the current backend tests do not instantiate a real Bevy `RenderDevice`.
