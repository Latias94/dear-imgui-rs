...
### 0.16 alpha.3 public API disposition
...
| Area | Provisional or current surface | Disposition | Alpha.3 contract |
| --- | --- | --- | --- |
...
| Font scale compatibility | `Io::font_global_scale` and `Io::set_font_global_scale` mirror a field that Dear ImGui moved to style state | delete | Read `Style::font_scale_main` and write it through `Style::set_font_scale_main`; owner-Context binding and numeric validation remain enforced there. |
| Color alpha-preview compatibility | `ColorEditFlags::ALPHA_PREVIEW`, `ColorPickerFlags::ALPHA_PREVIEW`, and `ColorButtonFlags::ALPHA_PREVIEW` incorrectly map upstream's zero-bit default to `ALPHA_NO_BG` | delete | Use `NONE` for the normal checkerboard alpha preview, `ALPHA_NO_BG` only to suppress the checkerboard, or `ALPHA_PREVIEW_HALF` for the split preview. |
| String compatibility names | The unused `ImStr` alias and allocating `im_str!` macro suggest a borrowed/native string contract they do not provide | delete | Use `&str`, `Cow<'_, str>`, or `ImString` directly. The existing ImString-backed input builders remain the zero-copy editing path. |
...
| Alpha.1 flow | Alpha.3 flow | New failure or cleanup state |
| --- | --- | --- |
