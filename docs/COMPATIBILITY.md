...
| Bevy native multi-viewport | `multi-viewport` implies `render` and `bevy_winit`; each Context opts in explicitly and secondary state is keyed by `(ContextId, ViewportId)`. Wayland falls back to host-window docking because global desktop client coordinates are unavailable; inspect `ImguiNativeViewportSupport` for the runtime status. |
