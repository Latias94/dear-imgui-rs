...
| dear-imgui-sys  | 0.8.x   | ImGui v1.92.5   | Docking branch via cimgui                 |
...
| dear-imgui-ash    | 0.8.x   | ash = 0.38             | Vulkan renderer (native only). Optional: `ash-window` for winit multi-viewport; SDL3 multi-viewport via `Platform_CreateVkSurface`; `gpu-allocator`/`vk-mem` allocators |
...
| Crate     | Version | External deps | Notes |
|-----------|---------|---------------|-------|
| dear-app  | 0.8.x   | winit, wgpu   | App runner (docking, themes, add-ons) |
...
| dear-imguizmo       | 0.8.x   | 0.8.x                  | dear-imguizmo-sys 0.8.x      | 3D gizmo                               |
